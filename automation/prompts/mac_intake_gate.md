...
If you duplicate this prompt into a Codex App Automation, replace `{{PYTHON_CMD}}` manually before creating the automation.

Goal: fetch exactly one published artifact on Mac, benchmark it, run quality gate, and record the decision.

Steps:
1. `git pull --ff-only origin main`
2. `{{PYTHON_CMD}} python_scripts/fetch_training_artifact.py --manifest automation/state/artifact_manifest.json --output-dir imports`
...
5. Record the decision with `{{PYTHON_CMD}} python_scripts/training_conveyor.py record-decision ...`
