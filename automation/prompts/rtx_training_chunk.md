Replace `{{DATASET_ROOT}}`, `{{BASE_CHECKPOINT}}`, and `{{CHUNK_EPOCHS}}` manually before creating the automation.
...
2. `./tracker_env/bin/python python_scripts/training_conveyor.py scan --dataset-root "{{DATASET_ROOT}}" --state-dir automation/state`
3. `./tracker_env/bin/python python_scripts/training_conveyor.py next-chunk --state-dir automation/state --base-checkpoint "{{BASE_CHECKPOINT}}" --chunk-epochs {{CHUNK_EPOCHS}} --write-plan automation/state/next_training_chunk.json --claim`
...
7. Publish it with `python_scripts/publish_training_artifact.py`.
8. Record the run with `python_scripts/training_conveyor.py record-run ...`.
