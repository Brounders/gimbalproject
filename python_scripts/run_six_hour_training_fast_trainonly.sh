...
SESSION_LOG="$LOG_DIR/${START_TS}_fast_trainonly.log"
...
if [[ ! -d "$NIGHT_DATASET" ]]; then
  echo "[ERROR] Night dataset not found: $NIGHT_DATASET" | tee -a "$SESSION_LOG"
  exit 2
fi
...
echo "[STEP] train 6h fast-full (train-only launcher) | model=$TRAIN_MODEL resume=${RESUME_FLAG:-false}" | tee -a "$SESSION_LOG"
