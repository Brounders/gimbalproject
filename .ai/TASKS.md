...
| A10 | Test coverage from ~15% → >30% — 🔄 In Progress. Cycle 1 done: DisplayStateTracker 21 tests (69 total). Remaining: config.py defaults, target_manager uncovered methods (update_from_yolo/age_targets), detectors (needs cv2 frames, deferred) | Medium | `tests/test_display_state_tracker.py` ✅; next: `test_config.py`, `tests/test_target_manager_lifecycle.py` |
