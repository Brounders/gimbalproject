...
| A10 | Test coverage → >30% — 🔄 In Progress. Cycle 1 ✅ DisplayStateTracker 21 tests. Cycle 2 ✅ config.py 67 tests (6 classes: construction/types/proportions/ordering/mutability/section-presence). Total: 136 tests. Remaining: target_manager lifecycle (update_from_yolo/age_targets), detectors (cv2 required — deferred) | Medium | `test_display_state_tracker.py` ✅ `test_config.py` ✅; next: `test_target_manager_lifecycle.py` |
